...
This module is the canonical vector-store implementation for the application.
The legacy `src.qdrant_vector_store` module remains as a compatibility wrapper.
"""
...
def make_stable_chunk_id(
    ...
    """Build a deterministic chunk id from repository-local chunk metadata."""
    raw = f"{repo_id}:{relative_path}:{start_line}:{end_line}:{text}"
    return hashlib.sha1(raw.encode("utf-8")).hexdigest()


def make_qdrant_point_id(chunk_id: str) -> str:
    """Convert a stable chunk id into a deterministic UUID for Qdrant."""
    return str(uuid.uuid5(uuid.NAMESPACE_URL, chunk_id))


def get_chunk_text(chunk: Any) -> str:
    """Read chunk text from supported chunk shapes."""
    return (
...
def get_chunk_metadata(chunk: Any) -> dict[str, Any]:
    """Read chunk metadata from supported chunk shapes."""
...
def get_metadata_value(
    ...
    *keys: str,
    ...
    """Read the first present key from a metadata mapping."""
    for key in keys:
...
class QdrantCodeVectorStore:
    """Qdrant-backed vector store for code and documentation chunks."""
    ...
    def reset_collection(self) -> None:
        """Delete only vectors for the current repo_id from the shared collection."""
